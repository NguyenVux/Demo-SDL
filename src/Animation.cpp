...
void AnimationInstance::Update() {
  ...
  if (m_accumulateTime > frameTime) {
    m_currentFrame = (m_currentFrame + m_accumulateTime / frameTime) %
...
AnimationInstance::AnimationInstance(const Animation *animation)
    : m_animation(std::move(animation)), m_currentFrame(0), m_accumulateTime(0),
      m_speed(1), m_isPlaying(0) {}
...
void AnimationInstance::Pause() { m_isPlaying = false; }

void AnimationInstance::Resume() { m_isPlaying = true; }

void Animation::SetName(const std::string &name) { m_name = name; }

void Animation::SetLoop(bool loop) { m_isLoop = loop; }

void Animation::SetSheet(const SpriteSheet *sheet) { m_sheet = sheet; }

Animation::Animation()
    : m_sheet(nullptr), m_fps(0), m_name(), m_isLoop(false) {}
Animation::Animation(const std::string &name, int fps, SpriteSheet *sheet,
                     bool isLoop)
    : m_name(name), m_fps(fps), m_sheet(sheet), m_isLoop(isLoop) {}
